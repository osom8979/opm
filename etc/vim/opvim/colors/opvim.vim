...
" - g:opvim_overrides          (default = {})
" - g:opvim_use_lowcolor_black (default = 0)
" - g:opvim_use_gui_italics    (default = 1)
" - g:opvim_use_term_italics   (default = 0)

let s:background_color = "151515"

if exists("g:opvim_overrides")
    let s:overrides = g:opvim_overrides
else
    let s:overrides = {}
endif

" Backwards compatibility
if exists("g:opvim_background_color")
            \ || exists("g:opvim_background_color_256")
            \ || exists("g:opvim_use_term_background_color")
    ...
    if exists("g:opvim_background_color")
        let s:overrides["background"]["guibg"] = g:opvim_background_color
    endif

    if exists("g:opvim_background_color_256")
        let s:overrides["background"]["256ctermbg"] = g:opvim_background_color_256
    endif

    if exists("g:opvim_use_term_background_color")
                \ && g:opvim_use_term_background_color
        let s:overrides["background"]["ctermbg"] = "NONE"
        let s:overrides["background"]["256ctermbg"] = "NONE"
    endif
endif

if exists("g:opvim_use_lowcolor_black") && g:opvim_use_lowcolor_black
...
fun! s:X(group, fg, bg, attr, lcfg, lcbg)
    ...
    if exists("g:opvim_use_term_italics") && g:opvim_use_term_italics
        let l:cterm_attr = l:attr
    else
        let l:cterm_attr = s:remove_italic_attr(l:attr)
    endif

    if !exists("g:opvim_use_gui_italics") || g:opvim_use_gui_italics
