...
OPT_DIR=/opt/opm/nginx-nextcloud-forward
if [[ -d "$OPT_DIR" ]]; then
...
DEFAULT_CONFIG_TEMPLATE=52-nginx-nextcloud-forward.conf
...
STACK_NAME=nginx-nextcloud-forward-$STACK_SUFFIX
COMPOSE_YML=52-nginx-nextcloud-forward.yml
echo "Deploy stack: $STACK_NAME"
docker stack deploy -c "$COMPOSE_YML" "$STACK_NAME"
CODE=$?

echo "Create NextCloud Forward-proxy server: Public(${FRONTEND_HOST}) -> Private(${BACKEND_SCHEME}://${BACKEND_HOST})"
